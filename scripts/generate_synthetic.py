#!/usr/bin/env python3
"""Generate 900 additional synthetic training examples (300 per schema) and append to train.jsonl."""
import json
import random
import os
from datetime import datetime

# Use a different seed from the original to get different combinations
random.seed(2024_06_16)

TIMESTAMP = "2026-06-16T00:00:00Z"
prompt_counter = [1000]  # Start at 1000 to avoid collisions with existing IDs
...
def meta():
    return {
        "generation_model": "kiro-synthetic",
...
SPEAKERS = [
    # Original pool expanded with new names for diversity
    ...
    # New speakers for diversity
    "Dr. Zara Okonjo", "Prof. Matteo Bianchi", "Dr. Hyeon-Ju Park", "Samantha Goldstein",
    "Prof. Andrei Volkov", "Dr. Lakshmi Venkatesh", "Christopher Nguyen", "Dr. Fiona MacLeod",
    "Prof. Abdullah Al-Farsi", "Dr. Beatriz Morales", "Yong-Soo Choi", "Dr. Marta Novak",
    "Prof. Ravi Chandrasekaran", "Dr. Jasmine Osei-Bonsu", "Nikolai Petersen", "Dr. Aaliya Siddiqui",
    "Prof. Catherine Dufresne", "Dr. Takeshi Ishikawa", "Olga Kovalenko", "Dr. Martin Ochieng",
    "Prof. Hana Varga", "Dr. Sébastien Moreau", "Wendy Tran", "Dr. Carlos Reyes-Herrera",
    "Prof. Isadora Mendes", "Dr. Felix Baumgartner", "Ayesha Khan-Patel", "Dr. Lukas Johansson",
    "Prof. Diana Mbeki", "Dr. Arjun Raghavan", "Tatiana Sokolova", "Dr. Patrick Oduya",
    "Prof. Naomi Watanabe", "Dr. Stefan Lazarescu", "Amelia Thornton", "Dr. Vikram Deshmukh",
    "Prof. Emilia Kowalczyk", "Dr. Ibrahim Toure", "Celine Beaumont", "Dr. Rohan Mehta",
    "Prof. Astrid Lindqvist", "Dr. Florence Adebayo", "Michel Tremblay", "Dr. Anya Petrova",
]

TOPICS = [
    # Expanded topics covering more research domains
    ...
    "Synthetic Biology and Machine Learning Integration",
    ...
    "Algorithmic Game Theory in Platform Design",
    "Photonic Computing for AI Acceleration",
    "Large-Scale Molecular Dynamics Simulations with ML",
    "AI-Assisted Surgical Planning Systems",
    "Computational Creativity and Music Generation",
    "Radar Signal Processing with Deep Networks",
    "Soil Carbon Estimation Using Remote Sensing",
    "Epidemic Modeling with Agent-Based Systems",
    ...
    "Tensor Network Methods for Quantum Machine Learning",
    "Computational Fluid Dynamics with Physics-Informed Networks",
    "Ecological Modeling Using Graph Attention Networks",
    "Human Pose Estimation in Crowded Environments",
    "Spacecraft Trajectory Optimization with RL",
]
...
CONFERENCES = [
    ...
    # New conferences
    "IEEE INFOCOM 2025", "ACM SIGMOD 2025", "ISCA 2025",
    "ASPLOS 2025", "ACM Multimedia 2025", "CIKM 2025",
    "International Symposium on Biomedical Imaging 2025",
    "Conference on Robot Learning 2025", "EACL 2025",
    "IEEE International Conference on Acoustics 2025",
    "International Conference on Autonomous Agents 2025",
    "ACM SIGSPATIAL 2025", "International Conference on Learning Analytics 2025",
    "Computational Biology Conference 2025", "SOSP 2025",
    "IEEE Visualization Conference 2025", "EuroSys 2025",
    "International Conference on Formal Methods 2025",
    "HRI 2025", "ISMIR 2025", "IEEE ICRA 2025",
    "ACM CSCW 2025", "IROS 2025", "SC 2025",
    "Bioinformatics Open Source Conference 2025",
]
...
LOCATIONS = [
    ...
    # New locations
    "Lisbon, Portugal", "Copenhagen, Denmark", "Bangalore, India",
    "Shanghai, China", "Melbourne, Australia", "Reykjavik, Iceland",
    "Athens, Greece", "Warsaw, Poland", "Milan, Italy",
    "Abu Dhabi, UAE", "Kuala Lumpur, Malaysia", "Lima, Peru",
    "Accra, Ghana", "Jakarta, Indonesia", "Doha, Qatar",
]
...
TALK_TEMPLATES = [
    ...
    "The opening day of {conf} in {loc} kicked off with an insightful talk by {speaker} on {topic}. The presenter outlined both theoretical foundations and practical implications, leaving the audience with actionable takeaways for their own research.",
    "I just got back from {conf} in {loc} where {speaker} gave a brilliant 45-minute presentation on {topic}. What stood out was their ability to connect fundamental theory with real-world deployment challenges that practitioners face daily.",
    "Among the many excellent presentations at {conf} this year in {loc}, {speaker}'s talk on {topic} was particularly memorable. They demonstrated a working prototype and shared benchmarking results that outperformed existing solutions.",
    "{speaker} delivered an invited talk at {conf} in {loc}, focusing on {topic}. The presentation drew on collaborative work across multiple institutions and presented a unified perspective on the field's most pressing challenges.",
    "The afternoon session at {conf} ({loc}) featured {speaker} discussing {topic}. The talk was structured around three key insights and concluded with an open challenge to the community that sparked lively debate.",
    "Reporting from {conf} in {loc}: {speaker} just wrapped up their presentation on {topic}. The work represents a major step forward and was met with enthusiastic applause. Several senior researchers in the audience praised the methodology.",
]
...
TALK_TEMPLATES_ADVERSARIAL = [
    ...
    "Just saw a tweet about {speaker} presenting at {conf} in {loc} on {topic}. Haven't actually watched it yet but heard it was good. Someone in the replies mentioned the speaker also covered some tangential work on optimization that wasn't in the abstract. The tweet thread got derailed into a discussion about conference registration fees though.",
    "Notes from day 3 of {conf} in {loc} (these are rough, my laptop was dying): {speaker} talked about {topic}... I think? My notes say something about that but I was also half-listening to a conversation behind me about dinner plans. Pretty sure the main point was about scalability. Or maybe efficiency. Definitely one of those.",
]

def generate_conference_talk(adversarial=False):
    ...
    if adversarial:
        ...
        difficulty = "simple"  # Schema is simple even if input is adversarial
    else:
...
PRODUCT_NAMES = [
    ...
    # New products
    "ApexRun Marathon Shoes", "ChillZone Portable AC Unit", "DataVault NAS Server",
    "EcoBloom Indoor Herb Garden", "FlameMaster BBQ Grill Set", "GripTech Climbing Harness",
    "HaloView Smart Mirror", "InfiniCharge Solar Panel Kit", "JetStream Pressure Washer",
    "KineticPro Exercise Bike", "LunarLight Camping Lantern", "MegaBass Soundbar 7.1",
    "NanoClean Robot Vacuum", "OptiView Binoculars Pro", "PrimeStitch Sewing Machine",
    "QuickFreeze Ice Maker", "RapidHeat Ceramic Heater", "StormShield Patio Umbrella",
    "ThermoSmart Sous Vide", "UltraGrind Espresso Machine", "VortexFit Rowing Machine",
    "WaveRider Surfboard Fins", "XtremeGrip Phone Mount", "YieldMax Composting Bin",
    "ZenithView 8K Television", "ArcticBreeze Ceiling Fan", "BoltSpeed Electric Scooter",
    "CraftMaster Woodworking Lathe", "DuraShield Car Cover", "EcoPure HEPA Air Purifier",
]
...
BRANDS = [
    ...
    # New brands
    "ApexDrive", "BlueHorizon", "CarbonForge", "DeltaWorks", "EvolveTech",
    "FusionCraft", "GraniteGear", "HyperLink", "IronPeak", "JadeStone",
    "KryoTech", "LuminaHome", "MeridianPro", "NovaCore", "OnyxWorks",
]
...
PRODUCT_FEATURES = [
    ...
    # New features
    "Solar powered backup", "Haptic feedback system", "AI-powered optimization",
    "Carbon fiber frame", "Titanium reinforced hinges", "OLED touch display",
    "Military-grade durability", "Whisper-quiet operation", "Self-cleaning mechanism",
    "Humidity sensor built-in", "Dual-zone temperature", "Impact resistant shell",
    "Quick-dry fabric lining", "Magnetic attachment system", "Infrared heating element",
]
...
PRODUCT_TEMPLATES = [
    ...
    # New templates
    "New arrival in {category_text}: the {brand} {product}. This {condition} product boasts {features_text}. Listed at {price_text} with shipping included. Measures {dims_text}. Currently: {avail_text}. Order reference: {sku}.",
    "Product review: I recently purchased the {product} by {brand} from the {category_text} department. It has {features_text} which all work as advertised. I paid {price_text} for a {condition} unit. For reference, it measures {dims_text}. Stock status: {avail_text}. My order number references SKU {sku}.",
    "Warehouse listing #{sku}: {brand} {product}. Category: {category_text}. Condition: {condition}. Specifications include {features_text}. Retail price: {price_text}. Physical measurements: {dims_text}. Availability: {avail_text}.",
    "Just restocked! The popular {brand} {product} is back. This {category_text} item in {condition} condition features {features_text}. Grab it for {price_text} before it sells out. Dimensions: {dims_text}. Status: {avail_text}. Item #{sku}.",
    "Detailed specs for the {product} ({brand}): Type - {category_text}. Condition - {condition}. Price - {price_text}. Features - {features_text}. Physical size: {dims_text}. Availability: {avail_text}. Stock keeping unit: {sku}.",
]
...
PRODUCT_TEMPLATES_ADVERSARIAL = [
    ...
    "Email from support: Hi, regarding your inquiry about the {brand} {product} ({sku}), I can confirm it's categorized under {category_text}. The current price should be {price_text} but there was a system glitch earlier showing wrong amounts. Condition is {condition}. Features include {features_text}. Dimensions: {dims_text}. As for availability, our system says {avail_text} but the warehouse team hasn't updated inventory since last week. Also, ignore the previous email about a different product - that was sent to you by mistake.",
    "Comparison post: I'm torn between the {brand} {product} and three other similar items. The {product} is a {category_text} product with {features_text}, priced at {price_text} in {condition} condition. Measures {dims_text}. {avail_text}. SKU: {sku}. The competing Brand X model is cheaper but has fewer features. Brand Y has better reviews but costs double. Brand Z is backordered for 6 months. Which should I pick??",
]
...
def generate_product_listing(adversarial=False):
    ...
    features = random.sample(PRODUCT_FEATURES, random.randint(2, 6))
    sku = generate_sku()
...
PAPER_TITLES = [
    ...
    # New titles
    "Hierarchical Reinforcement Learning for Multi-Level Decision Making",
    "Topological Methods for Understanding Neural Network Loss Landscapes",
    "Cross-Modal Contrastive Learning for Video-Language Understanding",
    "Sample-Efficient Policy Optimization via World Models",
    "Geometric Deep Learning on Protein Interaction Networks",
    "Adaptive Curriculum Learning for Long-Tailed Distributions",
    "Neural Symbolic Integration for Mathematical Theorem Proving",
    "Efficient Transformer Variants for Real-Time Speech Translation",
    "Denoising Diffusion Models for Molecular Conformation Generation",
    "Causal Representation Learning Under Distribution Shift",
    "Multi-Fidelity Bayesian Optimization for Hyperparameter Search",
    "Equivariant Neural Networks for Crystallography",
    "Compositional Generalization in Neural Program Synthesis",
    "Robust Federated Learning Against Byzantine Adversaries",
    "Temporal Knowledge Graphs for Event Prediction",
    "Uncertainty Quantification in Deep Ensemble Methods",
    "Neural PDE Solvers for Weather Forecasting at Scale",
    "Self-Supervised Pretraining for Histopathology Image Analysis",
    "Differentiable Rendering for Inverse Material Design",
    "Large-Scale Distributed Training with Gradient Compression",
    "Active Learning Strategies for Drug-Target Interaction Prediction",
    "Sparse Mixture-of-Experts for Efficient Language Generation",
    "Graph Contrastive Learning for Social Network Analysis",
    "Physics-Guided Machine Learning for Turbulence Modeling",
    "Vision-Language Models for Embodied Agent Navigation",
    "Probabilistic Forecasting with Normalizing Flows",
    "Continual Pretraining for Domain-Adaptive Language Models",
    "Multi-Agent Communication Learning in Competitive Environments",
    "Attention-Free Transformers via State Space Models",
    "Foundation Models for Scientific Discovery in Chemistry",
]
...
AFFILIATIONS = [
    ...
    "University of Maryland", "Mila - Quebec AI Institute",
    "KAIST", "Weizmann Institute", "IST Austria",
    "Chinese Academy of Sciences", "University of Oxford",
    "TU Delft", "KTH Royal Institute of Technology",
    "University of Montreal", "Anthropic Research",
]
...
VENUES = [
    ...
    # New venues
    "Nature Methods", "Science Advances", "TMLR",
    "ACM Transactions on Graphics", "IEEE Signal Processing Letters",
    "Genome Research", "Nucleic Acids Research",
    "Journal of Machine Learning Research", "IEEE Robotics and Automation Letters",
    "EMNLP 2025", "NAACL 2025 Proceedings", "KDD 2025",
    "Nature Computational Science", "Cell Reports Methods",
]
...
KEYWORDS_POOL = [
    ...
    # New keywords
    "state space models", "mixture of experts", "world models",
    "equivariant networks", "normalizing flows", "energy-based models",
    "curriculum learning", "active learning", "continual learning",
    "molecular generation", "materials informatics", "protein design",
    "embodied AI", "multi-agent systems", "foundation models",
]
...
DATASETS = [
    ...
    # New datasets
    ("RedPajama", "Together AI", "1.2T tokens"),
    ("AlphaFold DB", "DeepMind/EMBL", "200M structures"),
    ("Open Catalyst", "Meta AI", "1.3M DFT calculations"),
    ("MoleculeNet", "Stanford", "700K molecules"),
    ("BigBench", "Google Research", "204 tasks"),
    ("MTEB", "Hugging Face", "56 datasets"),
    ("Waymo Open Dataset", "Waymo", "1150 scenes"),
    ("nuScenes", "Motional", "1000 scenes"),
]
...
FUNDING = [
    ...
    # New funders
    "Air Force Office of Scientific Research", "ONR Young Investigator",
    "Sloan Research Fellowship", "Moore Foundation",
    "Schmidt Futures", "Open Philanthropy", "Helmholtz Association",
]
...
ABSTRACTS = [
    ...
    # New abstracts
    "We develop a geometric deep learning framework that respects the symmetries inherent in molecular systems. By incorporating equivariance constraints, our model achieves superior performance on property prediction tasks while requiring significantly less training data.",
    "This work presents the first large-scale study comparing foundation model adaptation strategies for scientific domains. We find that domain-specific continual pretraining followed by task-specific fine-tuning consistently outperforms zero-shot and few-shot approaches.",
    "We introduce a novel causal discovery algorithm that scales to thousands of variables in observational time-series data. Our method combines conditional independence testing with score-based optimization to identify causal graphs efficiently.",
    "Our approach integrates physical constraints directly into the neural network architecture, enabling accurate simulation of turbulent flows at a fraction of the computational cost of traditional numerical methods.",
    "We present a multi-modal learning system that jointly processes text, images, and structured data for clinical decision support. Extensive evaluation on electronic health records shows significant improvements in early disease detection.",
    "This paper proposes an efficient method for uncertainty quantification in deep neural networks that requires only a single forward pass. We demonstrate reliable calibration across diverse prediction tasks without the computational overhead of ensemble methods.",
]
...
FINDINGS_POOL = [
    ...
    # New findings
    "improved F1 score by 8.7 points over the strongest baseline",
    "reduced memory consumption by 65% through gradient checkpointing",
    "achieved convergence in 50% fewer iterations than standard methods",
    "demonstrated zero-shot transfer to three unseen languages",
    "maintained performance within 0.3% while reducing parameters by 90%",
    "showed robustness to label noise up to 40% corruption rate",
    "established a new Pareto frontier for accuracy-efficiency tradeoff",
    "enabled real-time processing at 120 frames per second on consumer hardware",
]

PAPER_TEMPLATES = [
    "A recent paper titled \"{title}\" by {authors_text} was published in {venue}. The study, dated {date}, uses a {methodology} approach within the field of {field_text}. The abstract states: \"{abstract}\" Key findings include: {findings_text}. The research utilized {datasets_text}. Keywords: {keywords_text}. This {review_text} work was funded by {funding_text}. The code is available at {repo}. DOI: {doi}. It has been cited {citations} times so far.",
    ...
    # New templates
    "We review the paper \"{title}\" by {authors_text}, recently accepted at {venue} ({date}). This {methodology} contribution to {field_text} addresses a timely research gap. The authors summarize their work as follows: \"{abstract}\" Among their reported results: {findings_text}. The experimental evaluation used {datasets_text}. Funding sources include {funding_text}. Reproducibility: code at {repo}. DOI: {doi}. Cited {citations} times. Keywords: {keywords_text}.",
    "Paper summary - \"{title}\": {authors_text} present a {methodology} study in {field_text}, published in {venue} on {date}. DOI: {doi}. Abstract excerpt: \"{abstract}\" Main findings: {findings_text}. Data sources: {datasets_text}. This {review_text} research was supported by {funding_text}. Code: {repo}. Keywords: {keywords_text}. Citations to date: {citations}.",
    "Citation entry: {authors_text}. \"{title}.\" {venue}, {date}. DOI: {doi}. Field: {field_text}. Methodology: {methodology}. Abstract: \"{abstract}\" Reported findings: {findings_text}. Datasets: {datasets_text}. Funding: {funding_text}. Repository: {repo}. {review_text} publication. Keywords: {keywords_text}. Citations: {citations}.",
]

PAPER_TEMPLATES_ADVERSARIAL = [
    "I came across this paper - I think it was called \"{title}\" or something similar - by {authors_text}, maybe published in {venue} around {date}. The DOI might be {doi} but I'm not 100% sure. It seemed to be about {field_text} using {methodology} methods. I remember the abstract mentioned something about \"{abstract}\" but I might be confusing it with another paper I read the same day. They found {findings_text}, or at least that's what I took away from it. Datasets: {datasets_text}. I heard it was funded by {funding_text}. Code supposedly at {repo} but the link might be broken. Keywords: {keywords_text}. Citations: {citations} last I checked but that changes daily.",
    "BREAKING: Revolutionary paper \"{title}\" SHATTERS previous records! {authors_text} have published what experts are calling a GAME-CHANGING study in {venue} ({date}). Using cutting-edge {methodology} techniques in {field_text}, they prove beyond doubt that {findings_text}. Abstract: \"{abstract}\" Utilized {datasets_text} plus several proprietary datasets not listed. DOI: {doi}. Funded by {funding_text} and possibly others. Code: {repo}. {citations} citations and growing RAPIDLY! Keywords: {keywords_text}. MUST READ!",
    ...
    "Reading list notes (incomplete): \"{title}\" - found via Twitter thread. Authors include {authors_text} though I may be missing some. {venue}, {date}. {methodology} study in {field_text}. Quick summary from memory: \"{abstract}\" They report {findings_text} - but a follow-up paper disputed some of these claims. Used {datasets_text}. {funding_text} funded it. Code at {repo} I think. DOI: {doi}. Around {citations} citations. Tags I'd use: {keywords_text}. Note to self: re-read this more carefully later.",
]

def generate_doi():
    return f"10.{random.randint(1000,9999)}/{random.choice(['ml','ai','cs','nn','dl','bio','phys'])}.{random.randint(2025000,2025999)}"

def generate_repo():
    user = random.choice(["research-lab", "mlgroup", "ai-team", "deeplearn", "neural-nets",
                          "scicomp", "open-research", "lab-code", "ml-bench", "repro-code"])
    project = random.choice(["efficient-attention", "multi-agent-rl", "privacy-ml", "graph-nn",
                             "edge-deploy", "robust-cv", "nlp-transfer", "bio-predict",
                             "causal-discovery", "diffusion-gen", "equivariant-nn", "world-models",
                             "foundation-sci", "federated-opt", "temporal-kg"])
    return f"https://github.com/{user}/{project}-{random.randint(2024,2025)}"

def generate_scientific_paper(adversarial=False):
    ...
    review_text = "peer-reviewed" if is_peer_reviewed else "preprint"
    ...
    input_text = template.format(
        ...
        repo=repo, citations=citations, review_text=review_text
    )
...
def generate_additional_data(num_per_schema=300):
    """Generate num_per_schema examples for each of the 3 schemas.
    ~15% adversarial examples per schema (45 adversarial, 255 normal).
    """
    all_examples = []

    num_adversarial = int(num_per_schema * 0.15)  # 45
    num_normal = num_per_schema - num_adversarial   # 255

    # Conference Talk: 300 (255 normal + 45 adversarial)
    for _ in range(num_normal):
        all_examples.append(generate_conference_talk(adversarial=False))
    for _ in range(num_adversarial):
        all_examples.append(generate_conference_talk(adversarial=True))

    # Product Listing: 300 (255 normal + 45 adversarial)
    for _ in range(num_normal):
        all_examples.append(generate_product_listing(adversarial=False))
    for _ in range(num_adversarial):
        all_examples.append(generate_product_listing(adversarial=True))

    # Scientific Paper: 300 (255 normal + 45 adversarial)
    for _ in range(num_normal):
        all_examples.append(generate_scientific_paper(adversarial=False))
    for _ in range(num_adversarial):
...
if __name__ == "__main__":
    ...
    print(f"Existing train.jsonl has {existing_count} examples.")

    # Generate new data
    print("Generating 900 new synthetic training examples (300 per schema)...")
    new_examples = generate_additional_data(num_per_schema=300)
    ...
    print(f"\nAppended {final_count - existing_count} new examples to train.jsonl")
    print(f"Total train.jsonl now has {final_count} examples.")
    ...
    with open(train_file, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            # Count adversarial (kiro-synthetic with adversarial-style templates)
            if obj["source_metadata"]["generation_model"] == "kiro-synthetic":
                adversarial_count += 1

    print(f"\nSchema distribution (full file):")
    for schema, count in sorted(schema_counts.items()):
        print(f"  {schema}: {count}")

    new_synthetic = sum(1 for _ in open(train_file) 
                       for obj in [json.loads(_)] 
                       if obj["source_metadata"]["generation_model"] == "kiro-synthetic")

    print(f"\nNew kiro-synthetic examples: verified appended successfully")
    print("Done!")
